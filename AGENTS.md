...
- `npx jest` — unit tests for `services/` and `utils/` (44 suites / 461 tests as of 2026-09-02); there is no `test` npm script. No lint script is configured — don't assume one is runnable. The rest of the testing plan (Maestro E2E, manual checklist) is agreed but **not started** (`docs/project-context.md` §12) — don't start it until the user says feature work is done.
...
- **Always deploy Edge Functions with `--use-api`**: `supabase functions deploy <fn> [<fn>...] --use-api`. Without the flag the CLI bundles inside a container that cannot see the project path here and fails with `entrypoint path does not exist` even though the file is right there. `--use-api` bundles server-side and correctly pulls in shared modules from `supabase/functions/_shared/`.
...
**Storage:** the Vault stores large files in **Backblaze B2**, but the app must never receive B2 credentials or upload tokens. `services/backblaze.ts` calls the `arquivos-b2` Edge Function, which validates auth/plan/storage limits and performs upload/delete server-side; download uses short-lived signed URLs. B2 secrets belong in Supabase secrets (`B2_KEY_ID`, `B2_APPLICATION_KEY`, `B2_BUCKET_ID`), never in `.env`, source code, or `EXPO_PUBLIC_*`.
